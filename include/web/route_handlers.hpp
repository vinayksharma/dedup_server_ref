...
public:
    static void setupRoutes(httplib::Server &svr, Status &status, Auth &auth)
    {
        ...
        svr.Post("/schedules", [&](const httplib::Request &req, httplib::Response &res)
                 {
            if (!AuthMiddleware::verify_auth(req, res, auth)) return;
        ...
        svr.Put("/schedules/([^/]+)", [&](const httplib::Request &req, httplib::Response &res)
                {
            if (!AuthMiddleware::verify_auth(req, res, auth)) return;
            handleUpdateSchedule(req, res); });

        svr.Delete("/schedules/([^/]+)", [&](const httplib::Request &req, httplib::Response &res)
                   {
            if (!AuthMiddleware::verify_auth(req, res, auth)) return;
            handleDeleteSchedule(req, res); });

        svr.Post("/schedules/([^/]+)/enable", [&](const httplib::Request &req, httplib::Response &res)
                 {
            if (!AuthMiddleware::verify_auth(req, res, auth)) return;
            handleEnableSchedule(req, res); });

        svr.Post("/schedules/([^/]+)/disable", [&](const httplib::Request &req, httplib::Response &res)
                 {
            if (!AuthMiddleware::verify_auth(req, res, auth)) return;
            handleDisableSchedule(req, res); });
    }
...
private:
    ...
    static void handleScan(const httplib::Request &req, httplib::Response &res)
    {
        ...
        try
        {
            ...
            // Create a scheduled scan for this directory
            auto &config_manager = ServerConfigManager::getInstance();
            int default_interval = config_manager.getDefaultScanInterval();

            auto &scheduler = ScanScheduler::getInstance();
            std::string schedule_id = scheduler.addSchedule(directory, default_interval, recursive, db_path);

            Logger::info("Created scheduled scan: " + schedule_id + " for directory: " + directory +
                         " (interval: " + std::to_string(default_interval) + "s)");
    ...
    static void handleCreateSchedule(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received create schedule request");
        try
        {
            auto body = json::parse(req.body);
            std::string directory = body["directory"];
            int interval_seconds = body.value("interval_seconds", 3600);
            bool recursive = body.value("recursive", true);
            std::string database_path = body.value("database_path", "scan_results.db");

            auto &scheduler = ScanScheduler::getInstance();
            std::string schedule_id = scheduler.addSchedule(directory, interval_seconds, recursive, database_path);
            ...
            res.set_content(response.dump(), "application/json");
            Logger::info("Schedule created successfully: " + schedule_id);
        }
        catch (const std::exception &e)
        {
            Logger::error("Create schedule error: " + std::string(e.what()));
            res.status = 400;
            res.set_content(json{{"error", "Failed to create schedule: " + std::string(e.what())}}.dump(), "application/json");
        }
    }

    static void handleGetSchedules(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received get schedules request");
        try
        {
            auto &scheduler = ScanScheduler::getInstance();
            ...
            for (const auto &schedule : schedules)
            {
                json schedule_json = {
                    ...
                    {"database_path", schedule.database_path},
                ...
                response["schedules"].push_back(schedule_json);
            }
        ...
        {
            Logger::error("Get schedules error: " + std::string(e.what()));
            res.status = 500;
            res.set_content(json{{"error", "Failed to retrieve schedules: " + std::string(e.what())}}.dump(), "application/json");
        }
    }

    static void handleGetSchedule(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received get schedule request");
        try
        {
            std::string schedule_id = req.matches[1];
            auto &scheduler = ScanScheduler::getInstance();

            if (!scheduler.scheduleExists(schedule_id))
            {
                res.status = 404;
                res.set_content(json{{"error", "Schedule not found: " + schedule_id}}.dump(), "application/json");
                return;
            }

            auto schedule = scheduler.getSchedule(schedule_id);
            json response = {
                {"id", schedule.id},
                {"directory", schedule.directory},
                {"interval_seconds", schedule.interval_seconds},
            ...
            auto last_run_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                                   schedule.last_run.time_since_epoch())
                                   .count();
            response["last_run"] = last_run_ms;

            auto next_run_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                                   schedule.next_run.time_since_epoch())
                                   .count();
            response["next_run"] = next_run_ms;

            res.set_content(response.dump(), "application/json");
            Logger::info("Schedule retrieved successfully: " + schedule_id);
        }
        catch (const std::exception &e)
        {
            Logger::error("Get schedule error: " + std::string(e.what()));
            res.status = 500;
            res.set_content(json{{"error", "Failed to retrieve schedule: " + std::string(e.what())}}.dump(), "application/json");
        }
    }

    static void handleUpdateSchedule(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received update schedule request");
        try
        {
            std::string schedule_id = req.matches[1];
            auto body = json::parse(req.body);
            ...
            auto existing_schedule = scheduler.getSchedule(schedule_id);

            ScanSchedule new_schedule;
            new_schedule.id = schedule_id;
            new_schedule.directory = body.value("directory", existing_schedule.directory);
            new_schedule.interval_seconds = body.value("interval_seconds", existing_schedule.interval_seconds);
            new_schedule.recursive = body.value("recursive", existing_schedule.recursive);
            new_schedule.database_path = body.value("database_path", existing_schedule.database_path);
            new_schedule.enabled = body.value("enabled", existing_schedule.enabled);

            bool success = scheduler.updateSchedule(schedule_id, new_schedule);
            if (!success)
            {
                res.status = 500;
                res.set_content(json{{"error", "Failed to update schedule"}}.dump(), "application/json");
                return;
            }
            ...
            res.set_content(response.dump(), "application/json");
            Logger::info("Schedule updated successfully: " + schedule_id);
        }
        catch (const std::exception &e)
        {
            Logger::error("Update schedule error: " + std::string(e.what()));
            res.status = 400;
            res.set_content(json{{"error", "Failed to update schedule: " + std::string(e.what())}}.dump(), "application/json");
        }
    }

    static void handleDeleteSchedule(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received delete schedule request");
        try
        {
            std::string schedule_id = req.matches[1];
            auto &scheduler = ScanScheduler::getInstance();
            ...
            bool success = scheduler.removeSchedule(schedule_id);
            if (!success)
            {
                res.status = 500;
                res.set_content(json{{"error", "Failed to delete schedule"}}.dump(), "application/json");
                return;
            }
            ...
            res.set_content(response.dump(), "application/json");
            Logger::info("Schedule deleted successfully: " + schedule_id);
        }
        catch (const std::exception &e)
        {
            Logger::error("Delete schedule error: " + std::string(e.what()));
            res.status = 500;
            res.set_content(json{{"error", "Failed to delete schedule: " + std::string(e.what())}}.dump(), "application/json");
        }
    }

    static void handleEnableSchedule(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received enable schedule request");
        try
        {
            std::string schedule_id = req.matches[1];
            auto &scheduler = ScanScheduler::getInstance();
            ...
            bool success = scheduler.enableSchedule(schedule_id);
            if (!success)
            {
                res.status = 500;
                res.set_content(json{{"error", "Failed to enable schedule"}}.dump(), "application/json");
                return;
            }
            ...
            res.set_content(response.dump(), "application/json");
            Logger::info("Schedule enabled successfully: " + schedule_id);
        }
        catch (const std::exception &e)
        {
            Logger::error("Enable schedule error: " + std::string(e.what()));
            res.status = 500;
            res.set_content(json{{"error", "Failed to enable schedule: " + std::string(e.what())}}.dump(), "application/json");
        }
    }

    static void handleDisableSchedule(const httplib::Request &req, httplib::Response &res)
    {
        Logger::trace("Received disable schedule request");
        try
        {
            std::string schedule_id = req.matches[1];
            auto &scheduler = ScanScheduler::getInstance();
            ...
            bool success = scheduler.disableSchedule(schedule_id);
            if (!success)
            {
                res.status = 500;
                res.set_content(json{{"error", "Failed to disable schedule"}}.dump(), "application/json");
                return;
            }
            ...
            res.set_content(response.dump(), "application/json");
            Logger::info("Schedule disabled successfully: " + schedule_id);
        }
        catch (const std::exception &e)
        {
            Logger::error("Disable schedule error: " + std::string(e.what()));
            res.status = 500;
            res.set_content(json{{"error", "Failed to disable schedule: " + std::string(e.what())}}.dump(), "application/json");
        }
    }
};
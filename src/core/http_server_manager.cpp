...
HttpServerManager::~HttpServerManager()
{
    try
    {
        // Only stop if we're still running to avoid double-stop issues
        if (running_.load())
        {
            Logger::debug("HttpServerManager: Destructor stopping server");
            stop();
        }
        else
        {
            Logger::debug("HttpServerManager: Destructor - server already stopped");
        }
    }
    catch (const std::exception &e)
    {
        // Swallow exceptions in destructor to prevent terminate
        Logger::error("HttpServerManager: Exception in destructor: " + std::string(e.what()));
    }
    catch (...)
    {
        // Swallow all exceptions in destructor
        Logger::error("HttpServerManager: Unknown exception in destructor");
    }
}
...
void HttpServerManager::stop()
{
    ...
    if (!running_.load())
    {
        Logger::debug("HttpServerManager: stop() called but server is not running");
        return;
    }

    Logger::debug("HttpServerManager: Stopping server...");
    ...
    try
    {
        if (server_)
        {
            Logger::debug("HttpServerManager: Calling server->stop()");
            server_->stop();
            Logger::debug("HttpServerManager: server->stop() completed");
        }

        if (server_thread_.joinable())
        {
            Logger::debug("HttpServerManager: Joining server thread");
            server_thread_.join();
            Logger::debug("HttpServerManager: Server thread joined successfully");
        }

        server_.reset();
        Logger::debug("HttpServerManager: Server instance reset");

        Logger::info("HttpServerManager: Server stopped");
    }
    catch (const std::exception &e)
    {
        Logger::error("HttpServerManager: Exception during stop: " + std::string(e.what()));
        // Continue with cleanup even if there was an exception
        try
        {
            if (server_thread_.joinable())
            {
                server_thread_.detach(); // Detach instead of join if there's an issue
            }
            server_.reset();
        }
        catch (...)
        {
            // Final fallback - just reset what we can
            server_.reset();
        }
    }
}
...
void HttpServerManager::reconfigureServer(const std::string &new_host, int new_port)
{
    ...
    try
    {
        ...
        Logger::info("HttpServerManager: Server configuration change audit - "
                     "Updated to " +
                     new_host + ":" + std::to_string(new_port) + " at " +
